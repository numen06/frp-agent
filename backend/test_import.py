...
def login():
    ...
    try:
        ...
        print("请确保服务器正在运行 (cd backend && python app.py)")
        sys.exit(1)
...
def test_import_ini(token):
    ...
    try:
        response = requests.post(
            f"{API_URL}/api/config/import/text", headers=headers, json=data, timeout=30
        )
...
def test_missing_auth(token):
    ...
    data = {"content": TEST_INI_CONFIG, "format": "ini", "frps_server_id": 1}
