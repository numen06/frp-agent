...
def get_current_user(request: Request, db: Session = Depends(get_db)) -> User:
    ...
    if not authorization:
        # 如果没有 Authorization Header，尝试从 URL 查询参数中获取 api_key（新增支持）
        api_key_from_url = request.query_params.get("api_key")
        if api_key_from_url:
            api_key_obj = verify_api_key(db, api_key_from_url)
            if api_key_obj:
...
def verify_credentials(credentials: HTTPBasicCredentials) -> bool:
    ...
    correct_password = secrets.compare_digest(
        credentials.password.encode("utf8"), settings.auth_password.encode("utf8")
    )
